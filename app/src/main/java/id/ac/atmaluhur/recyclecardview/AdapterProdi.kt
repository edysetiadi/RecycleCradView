package id.ac.atmaluhur.recyclecardview;
...
class AdapterProdi(val listProdi:ArrayList<Prodi>, val mode: String, val context: Context):
    RecycleView.Adapter<AdapterProdi.ViewHolder>() {
    ...
    overide fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        val konsentrasi = ""
        for(i in lp.konsentrasi,indinces) {
            konsentrasi += "${i + 1}. "
        ...
        with(holder) {
            cv.Prodi.setCardBackgroundColor(warna)
    ...
    overide fun getItemCount() = ListProdi.size
